    ...
    immer(
      (set) => ({
        ...
        addOutput: (parentId, parentTitle, newOutput) =>
          set((state) => {
            const parentOutput = state.output.children.find((child: { id: string; }) => child.id === parentId);
            if (!parentOutput) {
        ...
        addOrUpdateRootLeaf: (newLeaf) => {
          set((state) => {
            const idx = state.output.children.findIndex((child: { id: string; }) => child.id === newLeaf.id);
